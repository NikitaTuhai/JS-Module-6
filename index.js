...
class Hamburger {
  ...
  get calculatePrice() {
    ...
    const price = toppingPrice + this._size.price + this._stuffing.price;

    return price;
  }

  get calculateCalories() {
    const toppingCalories = this._toppings.reduce(
      (acc, topping) => acc + topping.calories,
      0
    );
    const calories =
      toppingCalories + this._size.calories + this._stuffing.calories;
    return calories;
  }
}
